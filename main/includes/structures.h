...
struct Dot_wav // Describes a .wav header, much redundant, but some useful
{
// From https://en.wikipedia.org/wiki/WAV 
// [Master RIFF chunk]
    const char FileTypeBlocID[4];   //  (4 bytes) : Identifier « RIFF »  (0x52, 0x49, 0x46, 0x46)
    const uint32_t FileSize;        //  (4 bytes) : Overall file size minus 8 bytes
    const char FileFormatID[4];     //  (4 bytes) : Format = « WAVE »  (0x57, 0x41, 0x56, 0x45)

// [Chunk describing the data format]
   const char FormatBlocID[4];      //    (4 bytes) : Identifier « fmt␣ »  (0x66, 0x6D, 0x74, 0x20)
   const uint32_t BlocSize;         //    (4 bytes) : Chunk size minus 8 bytes, which is 16 bytes here  (0x10)
   const uint16_t AudioFormat;      //    (2 bytes) : Audio format (1: PCM integer, 3: IEEE 754 float)
   const uint16_t NbrChannels;      //    (2 bytes) : Number of channels
   const uint32_t Frequency;        //    (4 bytes) : Sample rate (in hertz)
   const uint32_t BytePerSec;       //    (4 bytes) : Number of bytes to read per second (Frequency * BytePerBloc).
   const uint16_t BytePerBloc;      //    (2 bytes) : Number of bytes per block (NbrChannels * BitsPerSample / 8).
   const uint16_t BitsPerSample;    //    (2 bytes) : Number of bits per sample

// [Chunk containing the sampled data]
   const uint32_t DataBlocID;       //     (4 bytes) : Identifier « data »  (0x64, 0x61, 0x74, 0x61)
   const uint32_t DataSize;         //     (4 bytes) : SampledData size
   const uint16_t SampledData[0];   //      
};
